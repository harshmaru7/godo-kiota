...
// AccountRequestBuilder builds and executes requests for operations under \v2\account
...
func NewAccountRequestBuilderInternal(pathParameters map[string]string, requestAdapter i2ae4187f7daee263371cb1c977df639813ab50ffa529013b7437480d1ec0158f.RequestAdapter)(*AccountRequestBuilder) {
    m := &AccountRequestBuilder{
        BaseRequestBuilder: *i2ae4187f7daee263371cb1c977df639813ab50ffa529013b7437480d1ec0158f.NewBaseRequestBuilder(requestAdapter, "{+baseurl}/v2/account", pathParameters),
    }
    return m
}
...
// Get to show information about the current user account, send a GET request to `/v2/account`.The response includes the user's UUID, email, team membership, droplet/ volume/floating-IP quotas, and verification status. Authentication is required — a 401 response means the supplied bearer token is invalid or has been revoked.
// Deprecated: This method is obsolete. Use GetAsAccountGetResponse instead.
// returns a AccountResponseable when successful
...
func (m *AccountRequestBuilder) Get(ctx context.Context, requestConfiguration *AccountRequestBuilderGetRequestConfiguration)(AccountResponseable, error) {
    ...
    res, err := m.BaseRequestBuilder.RequestAdapter.Send(ctx, requestInfo, CreateAccountResponseFromDiscriminatorValue, errorMapping)
    if err != nil {
        return nil, err
    }
    if res == nil {
    ...
    return res.(AccountResponseable), nil
}
// GetAsAccountGetResponse to show information about the current user account, send a GET request to `/v2/account`.The response includes the user's UUID, email, team membership, droplet/ volume/floating-IP quotas, and verification status. Authentication is required — a 401 response means the supplied bearer token is invalid or has been revoked.
// returns a AccountGetResponseable when successful
// returns a Error error when the service returns a 401 status code
// returns a Error error when the service returns a 429 status code
// returns a Error error when the service returns a 500 status code
// returns a Error error when the service returns a 4XX or 5XX status code
func (m *AccountRequestBuilder) GetAsAccountGetResponse(ctx context.Context, requestConfiguration *AccountRequestBuilderGetRequestConfiguration)(AccountGetResponseable, error) {
    requestInfo, err := m.ToGetRequestInformation(ctx, requestConfiguration);
    if err != nil {
        return nil, err
    }
    ...
    res, err := m.BaseRequestBuilder.RequestAdapter.Send(ctx, requestInfo, CreateAccountGetResponseFromDiscriminatorValue, errorMapping)
    if err != nil {
        return nil, err
    }
    if res == nil {
    ...
    return res.(AccountGetResponseable), nil
}
// Keys the keys property
// returns a *AccountKeysRequestBuilder when successful
func (m *AccountRequestBuilder) Keys()(*AccountKeysRequestBuilder) {
    return NewAccountKeysRequestBuilderInternal(m.BaseRequestBuilder.PathParameters, m.BaseRequestBuilder.RequestAdapter)
}
// ToGetRequestInformation to show information about the current user account, send a GET request to `/v2/account`.The response includes the user's UUID, email, team membership, droplet/ volume/floating-IP quotas, and verification status. Authentication is required — a 401 response means the supplied bearer token is invalid or has been revoked.
